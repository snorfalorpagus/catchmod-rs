...
impl NonLinearStore {
    fn step(&self, inflow: f64, previous_outflow: f64) -> (f64, f64) {
        ...
        let q2 = if previous_outflow > 0.0 {
            if inflow < -ZERO {
                ...
                self.case_a(inflow, previous_outflow)
            }
        } else {
    ...
    fn case_a(&self, inflow: f64, previous_outflow: f64) -> f64 {
...
mod tests {
    ...
    fn zero_constant() {
        ...
        abs_diff_eq!(outflow, 999.0, epsilon = 0.0001);
    }
    ...
    fn case_d() {
        ...
        abs_diff_eq!(outflow, 29.9999, epsilon = 0.0001);
    }
}
